...
@RequestMapping(value = "/api/countries", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
...
public interface CountriesApi {
    ...
    @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = ResultData.class),
            examples = @ExampleObject(Examples.Country.Success.GET_ALL)))
    @GetMapping
    ResponseEntity<Object> getAll();

    @Operation(summary = "Returns a specific countries.")
    @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = ResultData.class),
            examples = @ExampleObject(Examples.Country.Success.GET)))
    @ApiResponse(responseCode = "404", content = @Content(schema = @Schema(implementation = Result.class),
            examples = @ExampleObject(name = "no record", description = "recordName: Country", value = Examples.Error.NOT_FOUND_BY_ID)))
    @GetMapping("/{id}")
    ...
    @ApiResponse(responseCode = "201", content = @Content(schema = @Schema(implementation = ResultData.class),
            examples = @ExampleObject(Examples.Country.Success.CREATE)),
